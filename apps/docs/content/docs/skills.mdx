...
The two are complementary. In Multica today, MCP support is **provider-specific**: Claude Code, CodeBuddy, Codex, DeepSeek, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, Qwen Code, Trae, and Grok consume `mcp_config`; other tools receive the field but don't actually use it. A dedicated MCP section will come in a later release.
...
- [AI coding tools comparison](/providers) — full comparison of all 18 tools (including each one's skill injection path)
