...
From DeepSeek. Multica exposes this as a separate provider while launching it through Codex app-server against DeepSeek's OpenAI-compatible API. The daemon detects `codex` for DeepSeek via `MULTICA_DEEPSEEK_PATH` (default `codex`), writes a per-task `$CODEX_HOME/config.toml` with `model_provider = "deepseek"`, `base_url = "https://api.deepseek.com"`, `env_key = "DEEPSEEK_API_KEY"`, and `wire_api = "chat"`, then removes Codex's inherited model cache so DeepSeek refreshes against the right catalog. Set `DEEPSEEK_API_KEY` in the agent custom env; use `MULTICA_DEEPSEEK_BASE_URL` only if you need a compatible proxy or alternate endpoint. The default and only built-in model is `deepseek-v4-flash`; set `MULTICA_DEEPSEEK_MODEL` to override the daemon-wide default. No DeepSeek secrets are written to repo files or Multica-managed config.
