...
| **DeepSeek** | DeepSeek | ✅ (Codex `thread/resume`) | ✅ | `$CODEX_HOME/skills/` | Static (`deepseek-v4-flash`) |
...
From DeepSeek. Multica exposes this as a separate provider while launching it through Codex app-server. Current Codex custom providers use the Responses wire protocol, while DeepSeek's official `https://api.deepseek.com` API is Chat Completions, so Multica does not point Codex directly at the official DeepSeek endpoint. Set `MULTICA_DEEPSEEK_BASE_URL` to a Responses-compatible adapter/router base URL; the daemon writes a per-task `$CODEX_HOME/config.toml` with `model_provider = "deepseek"`, that adapter `base_url`, `env_key = "DEEPSEEK_API_KEY"`, and `wire_api = "responses"`, then removes Codex's inherited model cache so DeepSeek refreshes against the right catalog. The adapter should accept Codex Responses requests, authenticate with the DeepSeek key Codex sends, forward to DeepSeek Chat Completions using `deepseek-v4-flash` unless `MULTICA_DEEPSEEK_MODEL` overrides it, and return Responses-format output to Codex. No DeepSeek secrets are written to repo files or Multica-managed config.
...
**Of the 18 tools, fourteen consume `mcp_config`: Claude Code, CodeBuddy, Codex, DeepSeek, Cursor, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, Qwen Code, Trae, and Grok**. The other four (Antigravity, Copilot, DevEco Code, Pi) accept the field but **ignore it** — no error, no warning, the config just has no effect.

The runtime paths are provider-specific: Claude Code, CodeBuddy, and Qwen Code receive it through daemon-owned temporary JSON files passed with `--mcp-config` (mode `0600` on POSIX systems because MCP configs may embed API tokens); Claude Code and CodeBuddy additionally use `--strict-mcp-config`; Codex and DeepSeek write a daemon-managed `mcp_servers` block into the per-task `$CODEX_HOME/config.toml`; Cursor writes `.cursor/mcp.json` plus per-task project approvals under `CURSOR_DATA_DIR`; Hermes, Kimi, Kiro CLI, Qoder, Trae, and Grok receive ACP `mcpServers`; OpenCode receives inline config through `OPENCODE_CONFIG_CONTENT`; OpenClaw receives `mcp.servers` through Multica's per-task config wrapper. OpenCode's path does **not** rewrite the project's `opencode.json`.
...
| DeepSeek | `$CODEX_HOME/skills/` | ✅ Native |
