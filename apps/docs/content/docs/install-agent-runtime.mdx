...
DeepSeek is exposed as its own Multica provider but runs through Codex app-server using DeepSeek's OpenAI-compatible endpoint. Install Codex first, then configure DeepSeek credentials on the agent or daemon environment.
...
| Authentication | Set `DEEPSEEK_API_KEY` where the daemon can pass it to the agent run. Agent custom env is preferred so the key stays scoped to DeepSeek agents. |
| Notes | Default model: `deepseek-v4-flash`. Override the daemon-wide default with `MULTICA_DEEPSEEK_MODEL`; override the endpoint with `MULTICA_DEEPSEEK_BASE_URL` only for a compatible proxy or alternate DeepSeek API base. |
