...
| `MULTICA_<PROVIDER>_MODEL` | empty | Default model for each AI coding tool. DeepSeek defaults to `deepseek-v4-flash` when unset |
| `MULTICA_DEEPSEEK_BASE_URL` | empty | Required Responses-compatible adapter/router base URL for DeepSeek via Codex. Current Codex cannot use DeepSeek's official Chat Completions endpoint directly |
| `DEEPSEEK_API_KEY` | empty | DeepSeek API key sent by Codex to the configured adapter via `env_key = "DEEPSEEK_API_KEY"`. Prefer setting it as agent custom env when only some agents need DeepSeek |
| `MULTICA_<PROVIDER>_ARGS` | empty | Daemon-wide default CLI arguments for a backend, applied to every task before each agent's own `custom_args`. Supported for `MULTICA_CLAUDE_ARGS`, `MULTICA_CODEX_ARGS`, `MULTICA_CODEBUDDY_ARGS`, and `MULTICA_QWEN_ARGS`; DeepSeek uses `MULTICA_CODEX_ARGS` because it launches through Codex app-server |
