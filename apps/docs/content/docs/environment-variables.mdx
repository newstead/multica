...
| `MULTICA_DEEPSEEK_BASE_URL` | `https://api.deepseek.com` | DeepSeek OpenAI-compatible API base URL used in the per-task Codex config. Override only for a compatible proxy or alternate endpoint |
| `DEEPSEEK_API_KEY` | empty | DeepSeek API key consumed by Codex via `env_key = "DEEPSEEK_API_KEY"`. Prefer setting it as agent custom env when only some agents need DeepSeek |
