...
type CodexHomeOptions struct {
	...
	// config.toml to DeepSeek's OpenAI-compatible endpoint. It is used only
...
const (
	...
	deepseekCodexDefaultBaseURL    = "https://api.deepseek.com"
...
func deepseekCodexBaseURL() string {
	if v := strings.TrimSpace(os.Getenv(deepseekCodexBaseURLEnv)); v != "" {
		return v
	}
	return deepseekCodexDefaultBaseURL
}
...
func ensureDeepSeekCodexProviderConfig(configPath string) error {
	...
	block := strings.Join([]string{
		...
		`name = "DeepSeek"`,
		`base_url = ` + strconv.Quote(deepseekCodexBaseURL()),
