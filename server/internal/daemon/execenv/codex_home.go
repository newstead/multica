...
type CodexHomeOptions struct {
	...
	// app-server but must not inherit the user's normal Codex/OpenAI provider.
	DeepSeekProvider bool
}
...
const (
	deepseekCodexProviderName      = "deepseek"
	deepseekCodexBaseURLEnv        = "MULTICA_DEEPSEEK_BASE_URL"
	deepseekCodexAPIKeyEnv         = "DEEPSEEK_API_KEY"
	deepseekCodexProviderBeginMark = "# BEGIN multica-managed deepseek-provider (do not edit; regenerated by daemon)"
	deepseekCodexProviderEndMark   = "# END multica-managed deepseek-provider"
)

var (
	deepseekCodexProviderBlockRe       = regexp.MustCompile(`(?ms)^` + regexp.QuoteMeta(deepseekCodexProviderBeginMark) + `.*?^` + regexp.QuoteMeta(deepseekCodexProviderEndMark) + `\n*`)
	deepseekCodexRootProviderLineRe    = regexp.MustCompile(`^\s*model_provider\s*=`)
	deepseekCodexProviderTableHeaderRe = regexp.MustCompile(`^\s*\[model_providers\.deepseek\]\s*(#.*)?$`)
	codexTomlTableHeaderRe             = regexp.MustCompile(`^\s*\[`)
)

func deepseekCodexBaseURL() (string, error) {
	if v := strings.TrimSpace(os.Getenv(deepseekCodexBaseURLEnv)); v != "" {
		return v, nil
	}
	return "", fmt.Errorf("%s is required for DeepSeek via Codex; set it to a Responses-compatible adapter/router base URL, not the official DeepSeek Chat Completions endpoint", deepseekCodexBaseURLEnv)
}

func stripDeepSeekCodexProviderConfig(existing string) string {
	existing = deepseekCodexProviderBlockRe.ReplaceAllString(existing, "")

	var out strings.Builder
	insideTable := false
	skippingDeepSeekTable := false
	for _, line := range strings.SplitAfter(existing, "\n") {
		trimmed := strings.TrimRight(line, "\r\n")
		if deepseekCodexProviderTableHeaderRe.MatchString(trimmed) {
			skippingDeepSeekTable = true
			insideTable = true
			continue
		}
		if skippingDeepSeekTable {
			if !codexTomlTableHeaderRe.MatchString(trimmed) {
				continue
			}
			skippingDeepSeekTable = false
		}
		...
		if !insideTable && deepseekCodexRootProviderLineRe.MatchString(trimmed) {
			continue
		}
...
func ensureDeepSeekCodexProviderConfig(configPath string) error {
	data, err := os.ReadFile(configPath)
	if err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("read config.toml: %w", err)
	}
	existing := stripDeepSeekCodexProviderConfig(string(data))
	baseURL, err := deepseekCodexBaseURL()
	if err != nil {
		return err
	}
	...
	block := strings.Join([]string{
		...
		`model_provider = "` + deepseekCodexProviderName + `"`,
		"",
		`[model_providers.` + deepseekCodexProviderName + `]`,
		`name = "DeepSeek"`,
		`base_url = ` + strconv.Quote(baseURL),
		`env_key = "` + deepseekCodexAPIKeyEnv + `"`,
		`wire_api = "responses"`,
...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	if opts.DeepSeekProvider {
		if err := ensureDeepSeekCodexProviderConfig(filepath.Join(codexHome, "config.toml")); err != nil {
			return fmt.Errorf("ensure deepseek codex provider config: %w", err)
		}
	}
	...
	if opts.DeepSeekProvider {
		if err := os.RemoveAll(filepath.Join(codexHome, codexModelsCacheFile)); err != nil {
			return fmt.Errorf("discard codex models cache for deepseek: %w", err)
		}
		if err := os.RemoveAll(filepath.Join(codexHome, codexModelsCacheBindingFile)); err != nil {
			return fmt.Errorf("discard codex models cache binding for deepseek: %w", err)
		}
	} else if err := syncCodexModelsCache(codexHome, sharedHome, freshHome); err != nil {
