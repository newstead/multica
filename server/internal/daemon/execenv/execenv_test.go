...
`
	...
	if !strings.Contains(s, "sandbox_workspace_write.network_access = true") {
		t.Error("missing dotted-key network_access = true")
	}
}
...
func TestCodexSandboxPolicyFor(t *testing.T) {
	...
	cases := []struct {
		...
		// Linux keeps workspace-write with network access: Landlock is not
		// affected by the macOS Seatbelt bug, and the per-task writable HOME
		// (MUL-4856) keeps host tooling working inside the sandbox.
...
// home wires the resolved policy all the way into config.toml. prepareCodexHome
// pins GOOS to linux, which resolves to workspace-write + network access on the
// fork (MUL-4856) — the managed block must carry that exact policy.
func TestPrepareCodexHomeWritesManagedSandboxBlock(t *testing.T) {
	...
	if !strings.Contains(s, `sandbox_mode = "workspace-write"`) {
		t.Errorf("config.toml missing workspace-write sandbox_mode, got:\n%s", s)
	}
	if !strings.Contains(s, "sandbox_workspace_write.network_access = true") {
		t.Errorf("config.toml missing network_access = true, got:\n%s", s)
	}
	if strings.Contains(s, "[sandbox_workspace_write]") {
		t.Errorf("managed block must use dotted-key form, not a table header, got:\n%s", s)
	}
}
