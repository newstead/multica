...
var probeAgentCLIs = func() map[string]AgentEntry {
	...
	// DeepSeek is routed through Codex app-server with a managed
	// OpenAI-compatible provider config that points at a Responses-compatible
	// DeepSeek adapter. It reuses the codex binary but must stay its own
	// provider key so the daemon can pin model_provider/base_url per task.
