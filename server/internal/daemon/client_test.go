	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			call: func(c *Client) error {
				return c.CompleteTask(context.Background(), "task-1", "done", "", "", "/tmp/wd", nil, false, "POISONED-S")
			},
		},
		{
			name:     "fail",
...
func TestTerminalReportsOmitEmptyRetiredSessionID(t *testing.T) {
	...
	if err := NewClient(srv.URL).CompleteTask(context.Background(), "task-1", "done", "", "sess-1", "/tmp/wd", nil, false, ""); err != nil {
