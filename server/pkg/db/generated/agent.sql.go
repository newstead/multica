...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, role_code, language_codes, language_policy
`
...
const clearAgentLanguageCodes = `-- name: ClearAgentLanguageCodes :one
...
func (q *Queries) ClearAgentLanguageCodes(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentLanguageCodes, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const clearAgentLanguagePolicy = `-- name: ClearAgentLanguagePolicy :one
...
func (q *Queries) ClearAgentLanguagePolicy(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentLanguagePolicy, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const clearAgentRoleCode = `-- name: ClearAgentRoleCode :one
...
func (q *Queries) ClearAgentRoleCode(ctx context.Context, id pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, clearAgentRoleCode, id)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
func (q *Queries) CreateAgentTask(ctx context.Context, arg CreateAgentTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, createAgentTask,
		...
		arg.PolicyModel,
		arg.PolicyThinkingLevel,
		arg.PolicyServiceTier,
		arg.PolicyRoleCode,
	)
	var i AgentTaskQueue
	err := row.Scan(
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, role_code, language_codes, language_policy FROM agent
WHERE id = $1
`
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, policy_model, policy_thinking_level, policy_service_tier, policy_role_code FROM agent_task_queue
WHERE id = $1
`
...
UPDATE agent SET
    ...
    role_code = COALESCE($19, role_code),
    language_codes = COALESCE($20::text[], language_codes),
    composio_toolkit_allowlist = COALESCE($21::text[], composio_toolkit_allowlist),
    language_policy = COALESCE($22, language_policy),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, role_code, language_codes, language_policy
`
