...
type RouterOptions struct {
	...
	MemoryMetrics   *obsmetrics.MemoryMetrics
...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	signupConfig := handler.Config{
		...
		MemoryHindsightAggregateTelemetryPath: strings.TrimSpace(os.Getenv("MULTICA_MEMORY_HINDSIGHT_AGGREGATE_TELEMETRY_PATH")),
		...
		MemoryMem0AggregateTelemetryPath:      strings.TrimSpace(os.Getenv("MULTICA_MEMORY_MEM0_AGGREGATE_TELEMETRY_PATH")),
		ServerVersion:                         normalizeServerVersion(version),
	}
	h := handler.New(queries, pool, hub, bus, emailSvc, store, cfSigner, analyticsClient, signupConfig, daemonHub)
	h.Metrics = opts.BusinessMetrics
	h.MemoryService.Telemetry = opts.MemoryMetrics
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/api/workspaces", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					...
					// Role policy matrix is member-visible; editing is admin-gated below.
					r.Get("/role-policy", h.GetWorkspaceRolePolicy)
				})
				// Admin-level access
				r.Group(func(r chi.Router) {
					...
					r.Put("/role-policy", h.UpdateWorkspaceRolePolicy)
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/api/ops/metrics", h.GetOpsMetrics)
			...
			r.Route("/api/dashboard", func(r chi.Router) {
				...
				r.Get("/agents/sessions", h.GetDashboardAgentSessions)
				r.Get("/agents/code", h.GetDashboardAgentCode)
