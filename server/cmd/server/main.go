...
func main() {
	...
	var memoryMetrics *obsmetrics.MemoryMetrics
	if metricsConfig.Enabled() {
		...
		memoryMetrics = metricsRegistry.Memory
	...
	if h.MemoryDeliveryWorker != nil {
		go h.MemoryDeliveryWorker.Run(sweepCtx)
	}
	...
	if h.MemoryDeliveryWorker != nil && !h.MemoryDeliveryWorker.WaitWithTimeout(5*time.Second) {
		slog.Warn("memory delivery worker did not exit within shutdown timeout")
	}
